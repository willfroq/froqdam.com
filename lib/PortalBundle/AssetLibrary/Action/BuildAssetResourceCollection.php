...
use Froq\PortalBundle\AssetLibrary\DataTransferObject\AssetResourceCollection;
...
final class BuildAssetResourceCollection
{
     ...
     * @throws \Exception
     */
    public function __invoke(SearchRequest $searchRequest, #[CurrentUser] User $user): AssetResourceCollection
    {
        $resultSet = ($this->getSearchResultSet)($searchRequest, $user, IndexNames::AssetResourceItem->readable());
        ...
        foreach ($resultSet?->getResults() ?? [] as $result) {
            ...
            if (!($item instanceof AssetResourceItem)) {
                continue;
            }

            $assetResourceItems[] = $item;
        }
