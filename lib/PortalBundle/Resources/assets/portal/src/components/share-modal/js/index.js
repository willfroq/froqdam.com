...
export class ShareModal extends UiComponent {
    ...
    handleStorageChange() {
        this.#getCurrentSelectedAssetResourceIdsCount()
    }
    ...
    #getCurrentSelectedAssetResourceIdsCount () {
        const storageKey = 'selectorStates';
        const states = localStorage.getItem(storageKey)
        let storedStates = JSON.parse(states) || [];
    ...
    updateCollectionCount () {
        const indicator = document.getElementById('share-button');
        const numberElement = indicator.querySelector('span');
        const storedStates = this.getAssetList()
