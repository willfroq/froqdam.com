...
export class AppFacets {
    ...
    setUrlParamsAsFilters() {
        ...
        if (parseInt(url.searchParams.get('size')) > 0) {
            let urlParamsObject = this.convertUrlParamsToObject(url.search);

            if (urlParamsObject.hasOwnProperty('filters')) {
                this.filters = urlParamsObject['filters'];
            }
    ...
    buildRequestUrl(requestUrl) {
        ...
        Object.keys(filters).forEach((key) => {
            let urlParam = this.protectedKeywords.includes(key) ? key : `filters[${key}]`;
